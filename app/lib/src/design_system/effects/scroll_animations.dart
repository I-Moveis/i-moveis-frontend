...
class _ScrollRevealState extends State<ScrollReveal>
  ...
  void _checkVisibility() {
    ...
    final visibleTop = max(0.0, screenPos.dy);
...
class _ParallaxAnimator extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double pixels = position.hasPixels ? position.pixels : 0.0;
    final double translation = pixels * factor;
...
class _StaggeredRevealState extends State<StaggeredReveal>
  ...
  Animation<double> _animationFor(int index) {
    ...
    if (totalUs == 0) return const AlwaysStoppedAnimation(1.0);
...
class _TextRevealState extends State<TextReveal>
  ...
  void _checkVisibility() {
    ...
    final visTop = max(0.0, screenPos.dy);
    final visBot = min(screenHeight, screenPos.dy + itemHeight);
    final visH = max(0.0, visBot - visTop);
